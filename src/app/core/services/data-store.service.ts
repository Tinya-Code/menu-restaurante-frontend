...
export class DataStoreService {
  ...
  readonly activeTemplateData = computed(() => this.activeRestaurantData()?.template_response?.data ?? null);
   ...
   */
  setTemplateResponse(response: any): void {
    this._data.update(current => {
      if (!current || current.restaurants.length === 0) return current;
      ...
      firstRes.template_response = response;
      updatedRestaurants[0] = firstRes;
